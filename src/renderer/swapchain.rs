...
impl Swapchain {
    pub fn new(instance: &ash::Instance, device: &Device, window: &Window, render_pass: vk::RenderPass) -> Self {
        let capabilities = window.surface_capabilities(device.physical_device);
        ...
        let image_views = Self::new_image_views(&images, &device.logical_device);
        let image_count = image_views.len();

        let (image_available_semaphores,
            render_finished_semaphores,
            start_draw_fences) = Self::new_syncs(image_count, &device.logical_device);

        let extent = capabilities.current_extent;

        let framebuffers = Self::new_framebuffers(&image_views, &device.logical_device, extent, render_pass);
    ...
    fn new_image_views(images: &Vec<vk::Image>, logical_device: &ash::Device) -> Vec<vk::ImageView> {
        ...
        for image in images {
            ...
            image_views.push(unsafe {
                logical_device.create_image_view(&info, None).unwrap()
            });
        }
    ...
    fn new_framebuffers(image_views: &Vec<vk::ImageView>, logical_device: &ash::Device, extent: vk::Extent2D, render_pass: vk::RenderPass) -> Vec<vk::Framebuffer> {
        ...
        for &image_view in image_views {
            ...
            framebuffers.push(unsafe {
                logical_device.create_framebuffer(&info, None).unwrap()
            });
        }
        framebuffers
    }

    fn new_syncs(image_count: usize, logical_device: &ash::Device) -> 
        ...
        for _ in 0..image_count {
            image_available_semaphores.push(unsafe {
                logical_device.create_semaphore(&semaphore_info, None).unwrap()
            });
            render_finished_semaphores.push(unsafe {
                logical_device.create_semaphore(&semaphore_info, None).unwrap()
            });
            start_draw_fences.push(unsafe {
                logical_device.create_fence(&fence_info, None).unwrap()
            });
        }
        (image_available_semaphores,
        render_finished_semaphores,
    ...
    pub unsafe fn cleanup(&mut self, logical_device: &ash::Device) {
        for i in 0..self.image_count {
            ...
            logical_device.destroy_fence(self.start_draw_fences[i], None);
            
            logical_device.destroy_framebuffer(self.framebuffers[i], None);
            logical_device.destroy_image_view(self.image_views[i], None);
        }
